...
class RegisterLoginTest {
  ...
  async testRegisterMultipleUsers(): Promise<void> {
    ...
    for (const bot of BOT_USERS) {
      try {
        ...
        const result = await apiService.register(bot.username, bot.password, bot.displayName);
  ...
  async testLoginMultipleUsers(): Promise<void> {
    ...
    for (const bot of BOT_USERS) {
      try {
        ...
        const result = await apiService.login(bot.username, bot.password);
  ...
  async testDuplicateRegistration(): Promise<void> {
    ...
    try {
      ...
      await apiService.register(firstBot.username, 'newpass123', 'New Name');
  ...
  async testInvalidLogin(): Promise<void> {
    ...
    for (const testCase of testCases) {
      try {
        ...
        await apiService.login(testCase.username, testCase.password);
  ...
  async testPasswordValidation(): Promise<void> {
    ...
    for (const testCase of testCases) {
      try {
        ...
        const result = await apiService.register(testCase.username, testCase.password, testCase.displayName);
        ...
        if (error.message.includes('Password must be at least 6 characters') && testCase.password.length < 6) {
  ...
  async testLogout(): Promise<void> {
    ...
    if (this.registeredUsers.length > 0) {
      ...
      try {
        ...
        await apiService.logout(testUser.id);
